...
function customise(title)
    ...
    if GetEntityModel(GetPlayerPed(-1)) == GetHashKey("mp_m_freemode_01") or GetEntityModel(GetPlayerPed(-1)) == GetHashKey("mp_f_freemode_01") then
        ...
        Menu.addOption("clothing_customise", function()
            if(Menu.ScrollBarString({"Ansigt","Maske","Hår","Arme","Bukser","Faldskærm","Sko","Tilbehør","T-shirt","Body armor","Mærker","Trøje/Jakke"}, componentScroller, function(cb)  componentScroller = cb end)) then
                subComponentScroller = GetPedDrawableVariation(GetPlayerPed(-1), componentScroller)
                textureScroller = GetPedTextureVariation(GetPlayerPed(-1), componentScroller)
                paletteScroller = GetPedPaletteVariation(GetPlayerPed(-1), componentScroller)
            end
        ...
        Menu.addOption("clothing_customise", function()
            if(Menu.ScrollBarInt("Nr", subComponentScroller, GetNumberOfPedDrawableVariations(GetPlayerPed(-1), componentScroller), function(cb)  subComponentScroller = cb end)) then
        ...
        Menu.addOption("clothing_customise", function()
            ...
            if(Menu.ScrollBarInt("Teksture", textureScroller, textureMax, function(cb)  textureScroller = cb end)) then
        ...
        --Menu.addOption("clothing_customise", function()
            --if(Menu.ScrollBarInt("Farver", paletteScroller, 2, function(cb)  paletteScroller = cb end)) then
                --SetPedComponentVariation(GetPlayerPed(-1), componentScroller, subComponentScroller, textureScroller, paletteScroller)
            --end
        --end)
        Menu.addOption("clothing_customise", function()
            if(Menu.Option("Fjern undertrøje"))then
       ...
       --end)
        ...
        paletteScroller = GetPedPaletteVariation(GetPlayerPed(-1), componentScroller)
        Menu.addOption("clothing_customise", function()
            local precomponentTable = {"Ansigt","Maske","Hår","T-shirt","Bukser","No idea","No idea","No idea","Tilbehør","No idea","No idea","No idea"}
        ...
        Menu.addOption("clothing_customise", function()
            if(Menu.ScrollBarInt("Teksture", textureScroller, GetNumberOfPedTextureVariations(GetPlayerPed(-1), componentScroller, subComponentScroller), function(cb)  textureScroller = cb end)) then
...
function accessories(title)
    ...
    if GetEntityModel(GetPlayerPed(-1)) == GetHashKey("mp_m_freemode_01") or GetEntityModel(GetPlayerPed(-1)) == GetHashKey("mp_f_freemode_01") then
        ...
        Menu.addOption("clothing_accessories", function()
            if(Menu.ScrollBarString({"Hjelm","Briller","Øreringe","Ukendt","Ukendt","Ukendt","Venstre håndled","Højre håndled"}, componentScroller, function(cb)  componentScroller = cb end)) then
                subComponentScroller = GetPedPropIndex(GetPlayerPed(-1), componentScroller)
                textureScroller = GetPedPropTextureIndex(GetPlayerPed(-1), componentScroller)
            end
        end)
        Menu.addOption("clothing_accessories", function()
            if(Menu.ScrollBarInt("Nr", subComponentScroller, GetNumberOfPedPropDrawableVariations(GetPlayerPed(-1), componentScroller), function(cb)  subComponentScroller = cb end)) then
        ...
        Menu.addOption("clothing_accessories", function()
            if(Menu.ScrollBarInt("Teksture", textureScroller, GetNumberOfPedPropTextureVariations(PlayerPedId(), componentScroller, subComponentScroller), function(cb)  textureScroller = cb end)) then
                SetPedPropIndex(GetPlayerPed(-1), componentScroller, subComponentScroller, textureScroller, false)
            end
        end)
        Menu.addOption("clothing_accessories", function()
            if(Menu.ScrollBarStringSelect({"Fjern hjelm","Fjern briller","Fjern æreringe","Fjern venstre håndled","Fjern højre håndled"}, removeScroller, function(cb)  removeScroller = cb end)) then
        ...
        local precomponentTable = {"Hjelm","Briller","Øreringe","Empty slot","Empty slot","Empty slot","Venstre håndled","Højre håndled"}
        local componentTable = {}
        for i = 0, 7 do
        ...
        Menu.addOption("clothing_accessories", function()
            if(Menu.ScrollBarStringSelect({"Fjern hjelm","Fjern briller","Fjern øreringe","Fjern venstre håndled","Fjern højre håndled"}, removeScroller, function(cb)  removeScroller = cb end)) then
                if removeScroller ~= 3 and removeScroller ~= 4 then
                    ClearPedProp(GetPlayerPed(-1), tonumber(removeScroller))
                elseif removeScroller == 3 then
                    ClearPedProp(GetPlayerPed(-1), 6)
